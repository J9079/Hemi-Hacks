...
export default function DonorDashboard() {
  ...
  const getStatusBadge = (status) => {
    switch (status) {
      ...
      case 'MATCHED':
        return <span className="px-2.5 py-1 text-xs font-bold rounded-full bg-purple-100 text-purple-800 animate-pulse">MATCHED</span>;
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-gradient-to-r from-orange-500/10 via-amber-500/5 to-transparent p-6 rounded-3xl border border-orange-100">
        <div>
          ...
          <p className="text-xs text-slate-600 mt-1">
            Dispatch surplus food instantly to nearby verified shelters across Ajmer.
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-sm">
          ...
          <span className="text-2xl sm:text-3xl font-black text-emerald-600 block mt-1">
            {stats.foodRescuedKg} <span className="text-sm font-bold text-slate-400">kg</span>
          </span>
          <span className="text-[11px] text-emerald-600 font-medium mt-1 block">~{stats.mealsRescued} meals served</span>
        </div>
      ...
      <div className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="p-6 border-b border-slate-100 flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-slate-500">Live rescue routing pipeline and status history</p>
          </div>
        </div>
